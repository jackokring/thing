...
public class ModItems {
    ...
    public static Item register(String name, Function<Item.Settings, Item> itemFactory, Item.Settings settings,
                                float compostChance, int fuelSeconds, boolean tooltip) {
        // Create the item key.
        ...
        if(compostChance > 0.0f)
            ...
            CompostingChanceRegistry.INSTANCE.add(SUSPICIOUS_SUBSTANCE, compostChance);
        ...
        if(fuelSeconds > 0)
            FuelRegistryEvents.BUILD.register((builder, context) ->
                    builder.add(SUSPICIOUS_SUBSTANCE, fuelSeconds * 20));

        if(tooltip)
            ItemTooltipCallback.EVENT.register((itemStack, tooltipContext, tooltipType, list) -> {
                if(!itemStack.isOf(item)) return;
                list.add(Text.translatable(item.getTranslationKey() + ".tooltip"));
            });
        return item;
    }
}
